define("cPanZoom",function() {
    ...
    $.data = $.data || function (obj, name, value) {
        var id = obj["panzoom"] = obj["panzoom"] || ++uuid;
        globalCache[id] = globalCache[id] || {};
        if (value) {
            globalCache[id][name] = value;
        } else {
            return globalCache[id][name];
        }
    }
     ...
     * Matrix functions adapted from Louis Remi's jQuery.transform                                                                                                                                 m.js)
    ...
    function Panzoom(elem, options) {
        ...
        // De-camelcase
        this._transform = "-webkit-transform";
    ...
    Panzoom.prototype = {
        ...
        setTransform: function (transform) {
            ...
            var i = $set.length;
            while (i--) {
                $[method]($set[i], '-webkit-transform', transform);
            }
        },
        ...
        getTransform: function (transform) {
            ...
            if (transform) {
                ...
                transform = $[this.isSVG ? 'attr' : 'style'](transformElem, '-webkit-transform');
            }
            ...
            if (transform !== 'none' && !rmatrix.test(transform)) {
                ...
                this.setTransform(transform = $.css(transformElem, '-webkit-transform'));
            }
        ...
        _startMove: function (event, touches) {
            ...
            if (touches && touches.length === 2) {
                ...
                move = function (e) {
                    e.preventDefault();
                    self.pan(
                        origPageX + e.touches[0].pageX - startPageX,
                        origPageY + e.touches[0].pageY - startPageY,
                        panOptions
                    );
                };
            }
    ...
    $.fn.panzoom = function (options) {
        ...
        if (typeof options === 'string') {
            ...
            // Or the value itself if there is only oneßß
            // Or keep chaining
            return ret.length ?
        ...
        $("<div class='panzoom'><div class='zoom-area' ><div class='zoom-inner'></div></div></div>").appendTo($("body"));

        var panZoomBox = $(".panzoom"),
            zoomInnerBox = $(".zoom-inner");

        this.bind("click", function (e) {
            zoomInnerBox.html("");
            zoomInnerBox.css("-webkit-transform","none");
            zoomInnerBox.css("transform","none");
            var img = new Image();
            img.src = this.src;
            img.onload = function () {
                zoomInnerBox.append(img);

            }
            panZoomBox.show();
        })

        panZoomBox.bind("click", function (e) {
        ...
        return this.each(function () {
            new Panzoom(zoomInnerBox[0], options);
        });
